 ...
 * @generated
 */
public class ProtocolCanonicalEditPolicy extends CanonicalConnectionEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		EObject nextValue;
		int nodeVID;
		for (Iterator values = ((Protocol) modelObject).getStates().iterator(); values
				.hasNext();) {
			nextValue = (EObject) values.next();
			nodeVID = ProVerVisualVisualIDRegistry.INSTANCE.getNodeVisualID(
		...
		for (Iterator values = ((Protocol) modelObject).getOperations()
		...
		try {
			...
			for (Iterator diagramLinks = existingLinks.iterator(); diagramLinks
				...
				EObject diagramLinkDst = nextDiagramLink.getTarget()
						.getElement();
				int diagramLinkVisualID = ProVerVisualVisualIDRegistry
	...
	private void collectAllLinks(View view) {
		...
		int diagramElementVisualID = ProVerVisualVisualIDRegistry
				.getVisualID(view);
	...
	private void storeTypeModelFacetLinks(EObject container,
		...
		if (ProvervisualPackage.eINSTANCE.getProtocol().isSuperTypeOf(
				containerMetaclass)) {
			for (Iterator values = ((Protocol) container).getOperations()
					.iterator(); values.hasNext();) {
				EObject nextValue = ((EObject) values.next());
				Object structuralFeatureResult = ((Operation) nextValue)
						.getEndState();
				if (structuralFeatureResult instanceof EObject) {
					EObject dst = (EObject) structuralFeatureResult;
					structuralFeatureResult = ((Operation) nextValue)
							.getStartState();
					if (structuralFeatureResult instanceof EObject) {
